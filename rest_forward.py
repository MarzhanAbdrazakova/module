...
class RestForwardAPI(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        mapper.connect('forward', uri,
                       controller=ForwardController, action='get_rules',
                       conditions=dict(method=['GET']),
                       requirements=requirements)

        mapper.connect('forward', uri,
                       controller=ForwardController, action='set_rule',
                       conditions=dict(method=['POST']),
                       requirements=requirements)

        mapper.connect('forward', uri,
                       controller=ForwardController, action='delete_rule',
                       conditions=dict(method=['DELETE']),
                       requirements=requirements)

        # for VLAN data
        uri += '/{vlanid}'
        mapper.connect('forward', uri, controller=ForwardController,
